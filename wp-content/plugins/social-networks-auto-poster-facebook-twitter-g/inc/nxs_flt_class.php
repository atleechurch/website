...
class nxs_Filters {
    ...
    public static function init($od=false) {
        $screen = get_current_screen(); //prr($screen->id );// prr($screen); var_dump($od);        toplevel_page_NextScripts_SNAP-network
        ...
        if($od || ( !empty($screen) && ( $screen->id == 'toplevel_page_nxssnap' || $screen->id=='settings_page_NextScripts_SNAP' || $screen->id =='toplevel_page_NextScripts_SNAP-network' || $screen->id == 'snapautoposter_page_nxssnap-settings' || $screen->id == 'snap_page_nxssnap-reposter' || $screen->id == 'snapautoposter_page_nxssnap-reposter' || $screen->id == 'snapautoposter_page_nxssnap-accounts'))) {
            ...
            self::$posts_types = array_merge( $builtin_types, $custom_types, array( 'nxs_filter' => 'nxs_filter' ) );
            ...
            self::$taxonomies         = array_merge( $builtin_taxonomies, $custom_taxonomies );
    ...
    public static function enqueue() {
        $screen = get_current_screen(); // prr($screen->id);
    ...
    public static function save_filter( $post_id ) {
        ...
        if ( get_post_type( $post_id ) === 'nxs_filter' ) {
            $count_compares          = intval( $_POST['nxs_count_compares'] );
   ...
   <div class="nxs_tls_bdX">
    ...
    public static function print_catsTags_section( $current_post, $metaSettings, $nt='', $ii='' ) { $ntN = $nt.$ii; // ## Categories and Tags
        ...
        $tags = get_tags( array( 'hide_empty' => false, 'include'=>implode(',',$metaSettings['nxs_tags_names']), 'number'=>500 ) );
        $categories = get_categories( array( 'hide_empty' => false, 'include'=>implode(',',$metaSettings['nxs_cats_names']), 'number'=>500 ) );
    ...
    public static function print_types_section( $current_post, $metaSettings, $nt='', $ii='' ) {   $ntN = $nt.$ii;
        ...
        echo '<h4 onclick="jQuery(\'#nxs_sec_types'.$ntN.'\').toggle();"; style="cursor:pointer; background-image: url(\''.NXS_PLURL.'img/icons/type24.png\');background-repeat: no-repeat; padding-top: 2px; padding-left: 28px; height:24px;">'.
          __( 'Type, Format', 'social-networks-auto-poster-facebook-twitter-g' ) .'&nbsp;&nbsp;&gt;&gt; </h4><div id="nxs_sec_types'.$ntN.'" class="nxsLftPad" style="display:'.($isVis?'block':'none').';">';
        ...
        */
        if( !empty( self::$posts_types ) ) {
            echo '<div><label class="field_title">'. __( 'Post Types', 'social-networks-auto-poster-facebook-twitter-g' ) . ':</label>&nbsp;&nbsp;<span class="description">'. __( 'Posts, Pages, Custom Post Types' ) .'</span><br/>';
    ...
    public static function print_meta_section( $current_post, $metaSettings, $nt='', $ii='' ) { $isVis = !empty($metaSettings['nxs_count_compares']);
    // ## Meta
        ...
        $count_compares   = (!empty( $current_post->ID))?$metaSettings['nxs_count_compares']:''; if(empty($count_compares)) $count_compares = 1; $relation_options = array( 'AND', 'OR' );
        ...
        for( $i = 1; $i <= $count_compares; $i++ ) { $postfix = $i == 1 ? '' : '_'. $i; $rel = $i == 1 ? '' : 'nxs_meta_compare_'. $i;
            ...
            echo '<div class="'. 'nxs_short_field" rel="'. $rel .'">';
            echo '<label class="field_title">'. __( 'Operator', 'social-networks-auto-poster-facebook-twitter-g' ) . ':</label>&nbsp;&nbsp;<span class="description">'. __( 'Compare Operator' ) .'</span>';
            self::print_select( (!empty( $current_post->ID))?$current_post->ID:0, $compare_options, "nxs_meta_operator$postfix", $metaSettings, true, false ); echo '</div>';
        ...
        echo '<input type="hidden" id="'. 'nxs_count_compares" name="'. 'nxs_count_compares" value="'. $count_compares .'">';

         echo "</div>";
}
    public static function print_taxonomies_sectionX( $current_post, $metaSettings, $nt='', $ii='' ) { $isVis = !empty($metaSettings['nxs_term_names']);
    ...
    public static function print_taxonomies_section( $current_post, $metaSettings, $nt='', $ii='' ) { $isVis = !empty($metaSettings['nxs_term_names']);
        //## Taxonomies
         echo '<h4 onclick="jQuery(\'#nxs_sec_taxonomies'.$nt.$ii.'\').toggle();"; style="cursor:pointer; background-image: url(\''.NXS_PLURL.'img/icons/tag24.png\');background-repeat: no-repeat; padding-top: 2px; padding-left: 28px; height:24px;" >'.
           __( 'Taxonomies', 'social-networks-auto-poster-facebook-twitter-g' ) .'&nbsp;&nbsp;&gt;&gt; </h4><div id="nxs_sec_taxonomies'.$nt.$ii.'" class="nxsLftPad" style="display:'.($isVis?'block':'none').';">';
        $terms_names = array(); $taxs_names = self::$taxonomies; $relation_options = array( 'AND', 'OR' ); $compare_options  = array( 'IN', 'NOT IN', 'AND' );
        $children_options = array( 'yes' => __( 'Yes', 'social-networks-auto-poster-facebook-twitter-g'), 'no' => __( 'No', 'social-networks-auto-poster-facebook-twitter-g') );
        $count_compares = !empty( $metaSettings['nxs_count_term_compares'])?$metaSettings['nxs_count_term_compares']:'';
        unset( $taxs_names['post_format'] );  $terms = get_terms( $taxs_names, array( 'hide_empty' => false ) );

        if( empty( $count_compares ) ) $count_compares = 1;  foreach( $terms as $term ) $terms_names[$term->taxonomy .'||'. $term->term_id] = $term->name; natsort( $terms_names );
        for( $i = 1; $i <= $count_compares; $i++ ) { $postfix = $i == 1 ? '' : '_'. $i;    $rel = $i == 1 ? '' : 'nxs_term_compare_'. $i; if( $i > 1 ) echo '<hr>';

          if( !empty( $taxs_names ) ) {
            echo '<div class="'. 'nxs_medium_field" rel="'. $rel .'">';
            echo '<label class="field_title">'. __( 'Taxonomies', 'social-networks-auto-poster-facebook-twitter-g' ) . ':</label>&nbsp;&nbsp;<span class="description">'. __( 'Select Taxonomies' ) .'</span>';
          ...
          if( !empty( $terms_names ) ) {
            ...
            self::print_select( (!empty( $current_post->ID))?$current_post->ID:0, $terms_names, 'nxs'.$nt.$ii."_term_names$postfix", !empty($metaSettings["nxs_term_names$postfix"])?$metaSettings["nxs_term_names$postfix"]:'', true, true, self::makeInputName("_term_names$postfix", $nt, $ii), 'nxs_term_names' );
            echo '</div>';
          }
            echo '<div class="'. 'nxs_short_field" rel="'. $rel .'">';
            echo '<label class="field_title">'. __( 'Operator', 'social-networks-auto-poster-facebook-twitter-g' ) . ':</label>&nbsp;&nbsp;<span class="description">'. __( 'Compare Operator' ) .'</span>';
            self::print_select( (!empty( $current_post->ID))?$current_post->ID:0, $compare_options, "nxs_term_operator$postfix", !empty($metaSettings["nxs_term_operator$postfix"])?$metaSettings["nxs_term_operator$postfix"]:'', false, false, self::makeInputName("_term_operator$postfix", $nt, $ii),'hui' ); echo '</div>';

            echo '<div class="'. 'nxs_short_field" rel="'. $rel .'">';
            echo '<label class="field_title">'. __( 'Child Terms', 'social-networks-auto-poster-facebook-twitter-g' ) . ':</label>&nbsp;&nbsp;<span class="description">'. __( 'Child Terms' ) .'</span>';
            self::print_select( (!empty( $current_post->ID))?$current_post->ID:0, $children_options, "nxs_term_children$postfix", !empty($metaSettings["nxs_term_children$postfix"])?$metaSettings["nxs_term_children$postfix"]:'', true, false, self::makeInputName("_term_children$postfix", $nt, $ii), 'hiu' ); echo '</div>';
        ...
        echo '<hr>';
        echo '<div class="'. 'nxs_short_field">';
        echo '<label class="field_title">'. __( 'Condition', 'social-networks-auto-poster-facebook-twitter-g' ) . ':</label>&nbsp;&nbsp;<span class="description">'. __( 'Terms Relation Condition' ) .'</span>';
        self::print_select( (!empty( $current_post->ID))?$current_post->ID:0, $relation_options, "nxs_term_relation", $metaSettings, false, false, self::makeInputName("_term_relation", $nt, $ii ), 'hiu'); echo '</div>';

        echo '<button data-ii="'.$ii.'" data-nt="'.$nt.'" id="'. 'nxs_add_term_compare">'. __( 'Add More', 'social-networks-auto-poster-facebook-twitter-g' ) .'...</button>';
        echo '<input type="hidden" id="'. 'nxs_count_term_compares" name="'.self::makeInputName('_count_term_compares', $nt, $ii).'" value="'. $count_compares .'">';
    ...
    public static function search_value_in_filter_meta( $post_id, $meta_name, $needle, $meta_value='' ) {
        if ( !empty($post_id) && (empty($meta_value)||(!is_array($meta_value)))) $meta_value = get_post_meta($post_id, $meta_name, true);
         if ($meta_name=='nxs_tax_names') { prr($meta_value); prr($needle); } //????
    ...
    public static function print_select( $post_id, $values, $title, $meta='', $print_key = false, $multiple = true, $name='', $class = 'nxsSelIt', $txnType='' ) { //if ($name=='') $name = $title;  prr($values); prr($meta);
    ?> <select name="<?php echo ( ($multiple ? $name .'[]' : $name) ); ?>" data-type="<?php echo $txnType; ?>" id="<?php echo $title; ?>" placeholder="<?php _e( 'Please select from the list...', 'social-networks-auto-poster-facebook-twitter-g' ) ?>" <?php echo ( $multiple ? 'multiple="multiple"' : '' ); ?> class="<?php echo $class; ?>" >

    <?php
        foreach( $values as $key => $option_item ) {
            $value = $print_key ? $key : $option_item; //prr($title); prr($value); prr($meta);
            $selected = self::search_value_in_filter_meta( 0, $title, $value, $meta ) ? 'selected="selected"' : '';
...
function get_posts_ids_by_filter( $filter ) { $args = array(); //prr($filter);
    ...
    if( !empty( $filter['nxs_tax_names'] ) || !empty( $filter['nxs_term_names'] ) || ( !empty( $filter['nxs_count_compares'] ) && $filter['nxs_count_compares'] > 1 )  ) {
      $term_compares = array(); $count_term_compares = empty( $filter['nxs_count_term_compares'] ) ? 1 : $filter['nxs_count_term_compares']; $term_compares['relation'] = empty( $filter['nxs_term_relation'] ) ? 'AND' : $filter['nxs_term_relation'];
      for( $i = 1; $i <= $count_term_compares; $i++ ) { $postfix = $i > 1 ? '_'. $i : ''; $new_term_compare = array();
        if( !empty( $filter["nxs_tax_names$postfix"] ) ) $new_term_compare['taxonomy'] = $filter["nxs_tax_names$postfix"];
        if( !empty( $filter["nxs_term_names$postfix"] ) ) $new_term_compare['terms'] = array_map( 'nxs_get_normalize_parameter', $filter["nxs_term_names$postfix"] );
        if( !empty( $filter["nxs_term_operator$postfix"] ) ) $new_term_compare['operator'] = $filter["nxs_term_operator$postfix"];
        $new_term_compare['include_children'] = empty( $filter["nxs_term_children$postfix"] ) ? true : $filter["nxs_term_children$postfix"];
        if( !empty( $new_term_compare ) ) $term_compares[] = $new_term_compare;
      } $args['tax_query'] = $term_compares;
    }
    ...
    if( !empty( $filter['nxs_meta_key'] ) || !empty( $filter['nxs_meta_value'] ) || ( !empty( $filter['nxs_count_compares'] ) && $filter['nxs_count_compares'] > 1 )  ) {
        $meta_compares = array(); $count_compares = empty( $filter['nxs_count_compares'] ) ? 1 : $filter['nxs_count_compares']; $meta_compares['relation'] = empty( $filter['nxs_meta_relation'] ) ? 'AND' : $filter['nxs_meta_relation'];
        for( $i = 1; $i <= $count_compares; $i++ ) { $postfix = $i > 1 ? '_'. $i : ''; $new_compare = array();
            if( !empty( $filter["nxs_meta_key$postfix"] ) ) $new_compare['key'] = $filter["nxs_meta_key$postfix"];
            if( !empty( $filter["nxs_meta_value$postfix"] ) ) { $values = $filter["nxs_meta_value$postfix"];
                $new_compare['value'] = array();
                foreach( $values as $value ) { $position_delimiter = mb_strrpos( $value, '||' );
                    $meta_value = get_post_meta( intval( mb_substr( $value, $position_delimiter + 2 ) ), mb_substr( $value, 0, $position_delimiter ), true );
                    $new_compare['value'][] = is_array( $meta_value ) ? serialize( $meta_value ) : $meta_value;
                }
            }
            if( !empty( $filter["nxs_meta_operator$postfix"] ) ) {
                switch( $filter["nxs_meta_operator$postfix"] ) {
                    case 'gt':
                        $new_compare['compare'] = '>';
                        break;
                    case 'gt=':
                    ...
                    case 'lt=':
                        ...
                        break;
                    default:
                        $new_compare['compare'] = $filter["nxs_meta_operator$postfix"];
                }
            }
            if( !empty( $filter["nxs_meta_type$postfix"] ) ) $new_compare['type'] = $filter["nxs_meta_type$postfix"];
            if( !empty( $new_compare ) ) $meta_compares[] = $new_compare;
