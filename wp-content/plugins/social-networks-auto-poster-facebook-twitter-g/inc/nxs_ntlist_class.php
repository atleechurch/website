...
class nxs_snapClassNT {
    ...
    public function showNTGroup() { $cbo = count($this->nt); $this->doAuth();  ?> <div class="nxs_box">
        <div class="nxs_box_header">
          <div class="nsx_iconedTitle" style="margin-bottom:1px;background-image:url(<?php echo NXS_PLURL;?>img/<?php echo $this->ntInfo['lcode']; ?>16.png);"><?php echo $this->ntInfo['name']; ?>
        ...
        <div class="nsx_tab_container"><?php /* ######################## Account Tab ####################### */ ?>
          ...
          <?php if (!$isNew && function_exists('_make_url_clickable_cb') ) { ?>
            <div id="nsx<?php echo $nt.$ii ?>_tab2" class="nsx_tab_content">
              <?php nxs_showNTFilters($nt, $ii, $options); nxs_addPostingDelaySelV4($nt, $ii, $options); $this->advTab(); ?>
   ...
   <div style="width:100%;"><strong><?php _e('Username:Password', 'social-networks-auto-poster-facebook-twitter-g'); ?>:</strong> </div><input name="<?php echo $nt; ?>[<?php echo $ii; ?>][proxyup]" style="width: 30%;" value="<?php echo htmlentities($options['proxy']['up'], ENT_COMPAT, "UTF-8"); ?>"/>
    ...
    public function elemKeySecret($ii,$lKey,$lSec,$key,$sec,$fnKey='appKey',$fnSec='appSec') { $nt = $this->ntInfo['lcode']; $ntU = $this->ntInfo['code']; ?>
      <div style="width:100%;"><strong><?php echo $lKey; ?>:</strong></div><input name="<?php echo $nt; ?>[<?php echo $ii; ?>][<?php echo $fnKey; ?>]" id="<?php echo $fnKey.$ii; ?>" style="width: 30%;" value="<?php echo htmlentities($key, ENT_COMPAT, "UTF-8"); ?>" />
      <div style="width:100%;"><strong><?php echo $lSec; ?>:</strong></div><input name="<?php echo $nt; ?>[<?php echo $ii; ?>][<?php echo $fnSec; ?>]" id="<?php echo $fnSec.$ii; ?>" style="width: 30%;" value="<?php echo htmlentities($sec, ENT_COMPAT, "UTF-8"); ?>" /><?php
    }
    ...
    public function elemMsgFormat($ii,$l,$fn,$val,$isVisible=true) { $nt = $this->ntInfo['lcode']; ?>
      <div class="nxsMsgFormatDiv" style="display:<?php echo ($isVisible)?"block":"none"; ?>;">
        <div style="width:100%;"><strong style="font-size: 15px;"><?php echo $l; ?>:</strong> (<a href="#" id="msgFrmt<?php echo $nt.$ii; ?>HintInfo" onclick="nxs_showHideFrmtInfo('<?php echo $fn.$nt.$ii; ?>'); return false;"><?php _e('Show format info', 'nxs_snap'); ?></a>)</div>
    ...
    public function elemTitleFormat($ii,$l,$fn,$val,$isVisible=true) { $nt = $this->ntInfo['lcode']; ?>
      <div class="nxsMsgTFormatDiv" style="display:<?php echo ($isVisible)?"block":"none"; ?>;">
        <div style="width:100%;"><strong id="altFormatText"><?php echo $l; ?>:</strong> </div><input name="<?php echo $nt; ?>[<?php echo $ii; ?>][<?php echo $fn; ?>]" id="<?php echo $nt.$ii; ?>SNAPformatT" style="width: 50%;" value="<?php echo htmlentities($val, ENT_COMPAT, "UTF-8"); ?>" onfocus="nxs_showFrmtInfo('msgFrmtT<?php echo $nt.$ii; ?>');" /><?php nxs_doShowHint("msgFrmtT".$nt.$ii); ?>
      </div><?php
    }
    ...
    public function saveCommonNTSettings($pval, $o) { if (isset($pval['do'])) $o['do'] = $pval['do']; else $o['do'] = 0;  if (isset($pval['nName'])) $o['nName'] = trim($pval['nName']); if (isset($pval['qTLng'])) $o['qTLng'] = trim($pval['qTLng']);
      ...
      while (isset($pval['nxs_tax_names_'.$jj])) {
    ...
    function nxs_tmpltAddPostMeta($post, $ntOpt, $pMeta){ $nt = $this->ntInfo['lcode']; $ntU = $this->ntInfo['code']; $ntName = $this->ntInfo['name']; $ii = $ntOpt['ii']; $doNT = $ntOpt['do']; ?>
      ...
      if ($post->post_status != "publish" && ((empty($pMeta) && !empty($ntOpt['fltrsOn']) && $ntOpt['fltrsOn']=='1')||($ntOpt['do']=='2'))){ ?>
